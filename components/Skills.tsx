...
export default async function Skills() {
  ...
  return (
    <div className=" px-4 lg:px-16 pt-7 pb-14">
      <div className="text-4xl md:text-6xl font-light">Magic Happens Here</div>
      {/* <div className="text-lg md:text-xl font-normal mt-1 lg:mt-2">
      ...
      <div className="grid grid-cols-4 md:grid-cols-6 justify-evenly items-center w-full text-center gap-4">
        {data.map((eachEle, index) => (
          <div className="flex flex-col w-max" key={eachEle.id}>
            <img
              alt={`${eachEle.title} logo`}
              src={eachEle.image}
              className="w-14 h-14 sm:w-20 sm:h-20 filter grayscale hover:grayscale-0 transition-filter duration-300 ease-out"
            />
            <h4>{eachEle.title}</h4>
          </div>
        ))}
